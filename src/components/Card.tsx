...
export function Card({ label, side, number, state, onChoose }: CardProps) {
  ...
  const drag = useRef({ startX: 0, startY: 0, dx: 0, dy: 0, active: false, moved: false })
  ...
  function pointerDown(e: ReactPointerEvent<HTMLDivElement>) {
    ...
    drag.current = { startX: e.clientX, startY: e.clientY, dx: 0, dy: 0, active: true, moved: false }
  ...
  function pointerUp() {
    ...
    const outward = side === 'left' ? d.dx < -SWIPE_THRESHOLD : d.dx > SWIPE_THRESHOLD
  ...
  return (
    <div
      ref={ref}
      className={`card${state === 'picked' ? ' card--picked' : ''}${state === 'faded' ? ' card--faded' : ''}`}
      role="button"
      tabIndex={0}
